...
def clean_stl_bus():
    df_file_list  =  [file for file in os.listdir(data_dict['raw']['df']['business_location'] ) if bool(re.search( "[0-9]", file))]
    df_list = [pd.read_excel(data_dict['raw']['df']['business_location'] + f'{file}').clean_names().assign(
