...
def clean_name_vectorized(column:pd.Series) -> pd.Series:
    ...
    if pd.api.types.is_string_dtype(column) is False:
        ...
        column = column.str.replace(r'^([&-,]\s?)(.+)$', r'\g<2>', regex=True)
        column = column.str.replace(r'^(of\s)(.+)$', r'\g<2>', regex=True)

        column = column.str.replace(r'\s{2,}', " ", regex=True,
                                                          flags=re.IGNORECASE)
        column = column.str.replace(r"\s-\s|\s-|-\s", "-", regex=True,
                                                          flags=re.IGNORECASE)
        column = column.str.replace(r'#\s', '#', regex=True, flags=re.IGNORECASE)
        column = column.str.replace(r'([0-9])(\s)([sthnd]{2})(\s)',
                                                          r'\g<1>\g<3>', regex=True,
                                                          flags=re.IGNORECASE)
        column = column.str.replace(r'([0-9]{1,})([abcefgijklmopqrsuvwxyz]{2,})',
                                                          r'\g<1> \g<2>', regex=True,
                                                          flags=re.IGNORECASE)
        column = column.str.replace(r'([^ ])(,|&)([ ])', r'\g<1>\g<2> \g<3>',
                                                          regex=True, flags=re.IGNORECASE)
        column = column.str.replace(r'([^ ])(,|&)([^\s])', r'\g<1>\g<2> \g<3>',
                                                                  regex=True, flags=re.IGNORECASE)
        column = column.str.replace(r'([a-z])(,)(a-z])', r'\g<1>\g<2> \g<3>',
                                                          regex=True, flags=re.IGNORECASE)
        column = column.str.replace(r'([a-z])(&)(a-z])', r'\g<1> \g<2> \g<3>',
                                                                  regex=True, flags=re.IGNORECASE)
        column = column.str.replace(r'(&)(a-z])', r'\g<1> \g<2>',
                                                                  regex=True, flags=re.IGNORECASE)
        column = column.str.replace(r'([a-z])(&)', r'\g<1> \g<2>',
                                                                  regex=True, flags=re.IGNORECASE)
        column = column.str.replace(r'([^#])([a-z]{1,})([0-9]{2,})',
                                                          r'\g<1>\g<2> \g<3>', regex=True,
                                                          flags=re.IGNORECASE)
        column = column.str.replace(r'([a-z]{2,})([#])', r'\g<1> \g<2>',
                                                          regex=True, flags=re.IGNORECASE)
        column = column.str.replace(r'\(.+?\)', "", regex=True,
                                                          flags=re.IGNORECASE)
        ...
        column = column.str.replace(r'\s{2,}', " ", regex=True,
                                                          flags=re.IGNORECASE)
        column = column.str.replace(r" -\s|\s-|-\s", "-", regex=True,
                                                          flags=re.IGNORECASE)
        column = column.str.replace(r'#\s', '#', regex=True, flags=re.IGNORECASE)
        column = column.str.replace(r'([0-9])(\s)([sthnd]{2})(\s)',
                                                          r'\g<1>\g<3>', regex=True,
...
def string_standardize_name(column:pd.Series) -> pd.Series:
    ...
    column = column.str.replace(r'\s{2,}', r' ', flags=re.IGNORECASE, regex=True)
    column = column.str.strip()
    column = column.str.lower()
    return column
...
def clean_name(series:pd.Series) -> pd.Series:
    """wrapper for cleaning an standardizing a name column"""
    # clean name_column
    series = clean_name_vectorized(series)
    # standardize new column
    series = string_standardize_name(series)
    return series
...
def parse_person(person_df, name_col, first_name_col, middle_name_col,
                 last_name_col, profession_col,suffix_col,alphabetized_col,
                 format_col=None, prefix=False, log=False):
    ...
    if format_col is None:
        ...
        format_col_new=format_col
        person_df[format_col_new] = person_df[format_col_new].fillna('lastName_firstName')
    ...
    person_df_full = person_df.drop(columns=[middle_name_col, suffix_col, profession_col]).copy(deep=True)
    initial_shape = person_df_full.shape[0]
    person_df = person_df.drop_duplicates(subset=[name_col,first_name_col, last_name_col])
    ...
    person_df[prefix + name_col] = combine_names(person_df[[prefix+first_name_col, prefix+last_name_col]], [prefix+first_name_col, prefix+last_name_col])
    ...
    person_df[prefix + name_col] = person_df[prefix + name_col].str.replace(' {2,}', ' ', regex=True)
...
def clean_business_name(business_name_col:pd.Series, numbers_format=False) -> pd.Series:
    ...
    business_name_col = business_name_col.str.replace(r'(\b)(the)(\b)', r'\g<1>', regex=True)
    business_name_col = business_name_col.str.replace(r'\s{2,}', ' ', regex=True)
    business_name_col = business_name_col.str.replace(r'\b(a-z)\s(a-z)\b', r' \g<1>\g<2> ', regex=True)
    business_name_col = business_name_col.str.replace(r'\b(tr)\b', r' trust ', regex=True)
    business_name_col = business_name_col.str.replace(r'\s{2,}', ' ', regex=True)
    business_name_col = business_name_col.str.replace(r'\b(mort|mrtg|mrtge|mtge)\b', 'mortgage', regex=True)
    business_name_col = business_name_col.str.replace(r'(.+)\s(#?0-9+)$', r'\g<1>', regex=True)
    business_name_col = business_name_col.str.strip()
    # replace empty strings w/ np.nan
    business_name_col = business_name_col.replace(r'^(\s+)?$', np.nan, regex=True)
    business_name_col = business_name_col.fillna('')
    if numbers_format is not False:
        # conditionally move numbers to the front i.e blue ave trust 112 -> 112 blue ave trust
        business_name_col = pd.Series(np.where(
            (numbers_format == 'numbers_last') & (business_name_col.str.contains(r'\s(0-9{2,}|0-9+\s0-9+)$')),
            business_name_col.str.replace(r'^(.+)\s(0-9+\s-{1,3}0-9+)$', r'\g<2> \g<1>', regex=True),
            business_name_col
        ))
        business_name_col = pd.Series(np.where(
            (numbers_format == 'numbers_last') & (business_name_col.str.contains(r'\s(0-9{2,}|0-9+\s0-9+)$')),
            business_name_col.str.replace(r'^(.+)\s(0-9+)$', r'\g<2> \g<1>', regex=True),
            business_name_col
    ))
    business_name_col = business_name_col.replace(r'\s{2,}', ' ', regex=True)
    business_name_col = business_name_col.replace(r'^(\s+)?$', np.nan, regex=True)
    return business_name_col
...
def make_business_main_type_col(business_name_col:pd.Series) -> pd.Series:
    if not pd.api.types.is_string_dtype(business_name_col):
        ...
        return (pd.Series(
            np.where(
                business_name_col.str.contains(
                    pat=r'\b(corp|inco?r?p?o?r?a?t?e?d?|i?n?corpo?r?a?t?i?o?n?|inc)\b', flags=re.IGNORECASE, na=False),
                'corporation',
                np.where(
                    business_name_col.str.contains(
                        pat=r'\b(corp|inco?r?p?o?r?a?t?e?d?|i?n?corpo?r?a?t?i?o?n?|inc)\b', flags=re.IGNORECASE,
                        na=False),
                    'corporation',
                    np.where(
                        (business_name_col.str.contains(pat=r'\b(llc)\b', flags=re.IGNORECASE, na=False)),
                        'llc',
                        np.where(
                            (business_name_col.str.contains(
                                pat=r'\b(ll?p|limited\spartnership|association|asso?c?i?a?t?e?s?|limited)\b',
                                flags=re.IGNORECASE, na=False)),
                            'partnership/association',
                            np.where(
                                business_name_col.str.contains(
                                    pat=r'\b(gov|government|commonwealth of ma|us postal)\b', flags=re.IGNORECASE,
                                    na=False),
                                "state or federal government",
                                np.where(
                                    (business_name_col.str.contains(
                                        pat=r'\b(condom?i?n?i?u?m? trust?|master\sdeed|condo)\b',
                                        flags=re.IGNORECASE, na=False)),
                                    "condo trust",
                                    np.where(
                                        (business_name_col.str.contains(
                                            pat=r'\b(trust?|irrevocable|revocable|living|nominee|tr)\b',
                                            flags=re.IGNORECASE, na=False)),
                                        'trust',
                                        np.where(
                                            (business_name_col.str.contains(
                                                pat=r'\b((city|town) of|^a-z+\s(a-z\s)?(city|town)$)\b',
                                                flags=re.IGNORECASE, na=False)),
                                            'municipality',
                                            np.where(
                                                (business_name_col.str.contains(
                                                    pat=r'\b(housing\sauthority|r?e?development|chamber\sof\scommerce)\b',
                                                    flags=re.IGNORECASE, na=False)),
                                                "quasi government",
                                                np.where(
                                                    (business_name_col.str.contains(
...
def make_business_sub_type_col(business_name_col:pd.Series) -> pd.Series:
    if not pd.api.types.is_string_dtype(business_name_col):
        ...
        return (pd.Series(
            np.where(
                (business_name_col.str.contains(
                    pat=r'\b(church|baptist|methodist|protestant|jewish|episcopal|buddhist)\b', flags=re.IGNORECASE,
                    na=False)),
                'religious',
                np.where(
                    (business_name_col.str.contains(pat=r'|'.join(bank_names), flags=re.IGNORECASE, na=False)),
                    'bank',
                    np.where(
                        (business_name_col.str.contains(
...
def make_business_short_name_col(business_name_col:pd.Series, business_sub_type_col:pd.Series,
    ...
    business_short_name_col = pd.Series(np.where((business_name_col.str.contains('federal home') &
...
def make_business_proper_name_col( business_name_col,business_short_name_col, business_main_type_col):
    ...
    business_proper_name_col = business_proper_name_col.str.replace('&', '', regex=True)
    business_proper_name_col = business_proper_name_col.str.replace(r' {2,}', r' ', regex=True)
    ...
    # business_proper_name_col = business_proper_name_col.str.replace(r'^(.+)(a-z)$', r'\g<1>')
    business_proper_name_col = business_proper_name_col.str.replace(r' {2,}', r' ', regex=True)
    business_proper_name_col = business_proper_name_col.str.strip()
    ...
    business_proper_name_col = pd.Series(np.where(business_main_type_col == 'municipality',
                                          business_proper_name_col.str.replace(
                                              ...
                                              r'everett|winthrop|meord|winchester|woburn'
    ...
    business_proper_name_col = business_proper_name_col.fillna(business_short_name_col.str.extract(r'^((\w+\s){1,2})(.+)$').iloc[:, 0])
    business_proper_name_col = business_proper_name_col.fillna(business_short_name_col.str.extract(r'^((\w+))(.+)$').iloc[:, 0],)
    business_proper_name_col = business_proper_name_col.fillna(business_name_col)
    return business_proper_name_col
...
def make_alphabetized_name(name_column):
    ...
    alphabetized_col = alphabetized_col.str.strip()
    return alphabetized_col
...
def parse_business(df, business_name_col, business_main_type_col = 'business_main_type', business_sub_type_col = 'business_sub_type',
    ...
    # subtype
    df[business_sub_type_col] = make_business_sub_type_col(business_name_col=df[business_name_col])
    # short name
    df[business_short_name_col] = make_business_short_name_col(business_name_col=df[business_name_col],
                                                               bank_names=bank_names,
                                                               business_sub_type_col=df[business_sub_type_col])
    # create proper name col (this col removes words such as realty, condo, holdings, etc from short name col)
    df[business_proper_name_col] = make_business_proper_name_col(
        business_name_col=df[business_name_col], business_short_name_col=df[business_short_name_col],
        business_main_type_col=df[business_main_type_col]
    )
...
#     df[typeCol] = classify_name(df, name_cols=[name_col],type_col=typeCol)
#     # fill na full names w/ concatenation of first and last name
#     df[name_col] = combine_names(df, name_cols=[firstName_col, lastName_col, initial_col])
#     # split dataframe into two based on typeCol
