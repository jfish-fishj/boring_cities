...
st_sfx = r'(aly|av|byu|blf|blvd|bd|bldg|rm|br|cswy|ctr|cir|ct|cr|cv|crk|cres|xing|curv|dr|est|expy|ext|frk|ft|fl|fwy|' \
         r'gdn|gtwy|hvn|' \
         r'hwy|hl|jct|ky|lks|ln|lgt|lp|mall|mnr|mdw|msn|mtn|pkwy|pass|path|pl|plz|pt|prt|pw|rst|rdge|rd|rm|rte|rw|shr|' \
...
def create_samples(phonebook):
    pdf2image.convert_from_path(phonebook, output_folder=classifier_files['sample images'], thread_count=4, fmt='png',
                                dpi=400)
...
def create_variables(df, text, return_vars=False):

    df = df[df[text].notnull()]
    with pd.option_context('mode.chained_assignment', None):
        stripped_text = 'stripped_' + text
        df[stripped_text] = df[text].str.replace('[^\x20-\x7E]', ' ', flags=re.IGNORECASE).str.replace(
            r'(\s{2,}|\t)', ' ').str.strip()
        df[stripped_text] = df[stripped_text].str.replace('^\*', '', flags=re.IGNORECASE).str.replace(
            r'(\s{2,}|\t)', ' ').str.strip()
        og_vars = df.columns
        df['all_caps'] = np.where(
            df[stripped_text].str.contains(r'^[A-Z\s&-]+$'), 1, 0
        )
        df['contains_number'] =  np.where(df[stripped_text].str.contains('[0-9]'), 1, 0)
        ...
        df['starts_with_street_suffix'] = np.where(
            df[stripped_text].str.contains(f'^{st_sfx} ', flags=re.IGNORECASE), 1, 0)
        df['ends_with_street_suffix'] = np.where(df[stripped_text].str.contains(f' {st_sfx}$', flags=re.IGNORECASE),
                                                 1, 0)
        df['contains_CONT_AND'] = np.where(df[stripped_text].str.contains('CONTD|AND'), 1, 0)
        df['contains_PHONE'] = np.where(df[stripped_text].str.contains('PHONE'), 1, 0)
        df['number_spaces'] = df[stripped_text].str.count(' ')
    ...
    if return_vars is not False:
        return df, [col for col in df.columns if col not in og_vars]
    else:
        return df
...
if __name__ == "__main__":
    ...
    df = pd.read_csv(
        '/Volumes/Seagate Portable Drive/boring_cities/data/raw/sf/phonebook/training_data/training_data.csv',
        encoding="ISO-8859-1")
    df = df[df['classification'].isin(['complete', 'ignore', 'header', 'partial'])]

    classifified = create_variables(df=df, text='text', return_vars=True)
    new_df = classifified[0]
    factor = pd.factorize(new_df['classification'])
    definitions = factor[1]
    new_df.classification = factor[0]
    model = RandomForestClassifier()
    vars = classifified[1]
    X_train, X_test, y_train, y_test = train_test_split(new_df[vars], new_df['classification'], test_size=0.25
                                                        )
    model.fit(X=X_train[vars], y=y_train)
    with open(classifier_files['random forest'], 'wb') as f:
        joblib.dump(model, f)
